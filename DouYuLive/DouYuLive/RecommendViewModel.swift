...
extension RecommendViewModel {
    ...
    func requestData(completionHandler: @escaping () -> () ) {
        ...
        let parameters = ["limit": "4", "offset": "0", "time": NSDate.currentTime()]
        
        let dispatchGroup = DispatchGroup()
        // 1. 请求推荐数据
        // https://capi.douyucdn.cn/api/v1/getbigDataRoom?time=1474252024
        dispatchGroup.enter()
        NetworkTools.requestData(type: .GET, URLString: "https://capi.douyucdn.cn/api/v1/getbigDataRoom", parameters: ["time": NSDate.currentTime()]) { (result) in
        ...
        NetworkTools.requestData(type: .GET, URLString: "https://capi.douyucdn.cn/api/v1/getHotCate", parameters: parameters) { (result) in
            ...
            for dic in dataArray {
                ...
                // 妈的坑死我了 第三组老没有数据卧槽 结果是人家数据里本来room_list就是空的 没有
            }
        ...
        dispatchGroup.notify(queue: DispatchQueue.main) {
            ...
            self.anchorGroups.remove(at: 3)  // 由于有错误数据 所以 删除这一个数据
            completionHandler()
        }
    }
